    ...
    <form action="{{route('fornecedores.update', ['id' => $fornecedor->id])}}" method="post">
        ...
        <div class="row">
            ...
            <div class="col">
                ...
                <input type="text" name="telefone" id="telefone" class="form-control @error('telefone') is-invalid @enderror" aria-label="Small" aria-describedby="inputGroup-sizing-sm" value="{{$fornecedor->telefone}}">
                @error('telefone')
                    <div class="invalid-feedback">
                    {{$message}}
                    </div>
